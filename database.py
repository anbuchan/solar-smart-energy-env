...
def get_history(limit=100):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    # Sort by timestamp DESC (latest runs first) and THEN by step ASC (0 to 23 in order)
    ...
    for row in rows:
        ...
        # Keep them as strings for the UI to prevent [object Object]
        # But ensure they are clean and readable
        try:
            # If it's already a string from the DB, we just leave it or beautify it
            # We'll parse and re-dump to ensure it's a clean string format
